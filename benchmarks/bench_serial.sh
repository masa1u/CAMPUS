...
# パラメータのデフォルト値
DATASET_TYPES=("siftsmall" "sift" "gist")
...
run_benchmarks() {
    ...
    for i in {1..3}; do
        # 実行コマンド
        CMD="./bench_serial --dataset_type=${dataset_type} --initial_num=${initial_num} --posting_limit=${posting_limit} --connection_limit=${connection_limit} --insert_threads=${insert_threads} --search_threads=${search_threads} --top_k=${top_k} --node_num=${node_num} --pq_size=${node_num} --output_file=${timestamp}"
...
for dataset_type in "${DATASET_TYPES[@]}"; do
    if [ "$dataset_type" == "siftsmall" ]; then
